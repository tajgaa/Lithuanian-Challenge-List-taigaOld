function userList() {         //vardai
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        var p = roundNumber(getUserPoint(i+1, 100, entry.percentToQualify, "144hz") * 1, 2); //verifikacija
        if (checkVerify == true) {
            for (var b = 0 ; b < user_data.length ; b++) {
        ...
        for (var a = 0 ; a < entry.vids.length ; a++) {
            ...
            var p = getUserPoint(i+1, entry2.percent, entry.percentToQualify, entry2.hz); //completionas
            if (isLoot == true) {
                for (var b = 0 ; b < user_data.length ; b++) {
                    ...
                    if (user_name == data_name) {
                        ...
                        user_data[b].progress.push({map : entry.name.toString(), progress : entry2.percent.toString(), link : entry2.link, score : roundNumber(p,2), rank : i+1, hz : (entry2.hz != null ? entry2.hz : "144hz")});
                    }
                }
            } else {
                ...
                var prog = [];
                prog.push({map : entry.name.toString(), progress : entry2.percent.toString(), link : entry2.link, score : roundNumber(p,2), rank : i+1, hz : (entry2.hz != null ? entry2.hz : "144hz")});
    ...
    for (var i = 0 ; i < user_data.length ; i++) {        // tasku rodymas
        user_data[i].point = roundNumber(user_data[i].point, 2);
...
function getUserData(user) {
    ...
    for (var i = 0 ; i < user_data[user].verified.length ; i++) {
        ...
        clears++;
        progresses = progresses + '<li>' + list[rank].name + ' Verified </strong>(#'+(rank+1)+' / UP: '+roundNumber(getUserPoint(rank+1, 100, list[rank].percentToQualify, "")*1, 2)+')<strong></a></li>'
    }
    ...
    Swal.fire({
        ...
        confirmButtonColor: '#374234',
        html : '<center><strong>Taškų kiekis : '+user_data[user].point + '<br>'+
            'Geriausias rekordas : '+user_data[user].highest+'<br>'+
            'Įveikti lygiai : '+clears+' '+
            '<br>Rekordų sąrašas : '+progresses+'<br>'+
            '</strong></center>'
